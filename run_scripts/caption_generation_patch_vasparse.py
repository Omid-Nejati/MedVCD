...
for idx, img_id in tqdm(enumerate(range(len(img_files))), total=len(img_files)):
    ...
    def patching (model):
        ...
        if vasparse_contrastive_decoding:
            ...
            setattr(model.llama_model, 'vasparse_search_contrastive_decoding', vasparse_search_contrastive_decoding.__get__(model.llama_model)) #vasparse_decoding
    ...
    if prefill_sparse:
        llamamodel_patching(model, model_config)
    else:
        llamamodel_patching(model, model_config, [])
    ...
    findings_kwargs['prompt_length_image_text'] = 623
