...
from minigpt4.sparse.LlamaModelPatching import llamamodel_patching,llamamodel_patching_mplug_owl2
from minigpt4.sparse.LlamaModelPatchingTimer import llamamodel_patching_timer
...
if prefill_sparse:
    print("----------------prefill_sparse = True-----------------")
else:
    print("----------------prefill_sparse = False-----------------")
...
for idx, img_id in tqdm(enumerate(range(len(img_files))), total=len(img_files)):
    ...
    def patching (model):
        setattr(model.model, "generate", generate.__get__(model.model))
        ...
        if vasparse_contrastive_decoding:
            ...
            from decoder_zoo.VASparse.vasparse_decoding_mplugowl2 import  vasparse_search_contrastive_decoding as vasparse_decoding_mplugowl2
            setattr(model.model, "generate", vcs_generation.__get__(model.model))
            setattr(model.model, 'vasparse_search_contrastive_decoding', vasparse_decoding_mplugowl2.__get__(model.model)) #vasparse_decoding
            setattr(model.model, '_update_model_kwargs_for_vasparse_contrastive_decoding', _update_model_kwargs_for_vasparse_contrastive_decoding.__get__(model.model)) #vasparse_decoding

    if prefill_sparse:
        llamamodel_patching_mplug_owl2(model, model_config)
    else:
        llamamodel_patching_mplug_owl2(model, model_config, [])
    ...
    findings_kwargs['prompt_length_image_text'] = 82
    ...
    generated_captions_path = os.path.join(
        base_dir,
        f"{model_name}_{decoding_strategy}_{detector_type}_box_{box_threshold}_beams_{num_beams}_k_{k_candidate_num}_{dataset_name}_expand_ratio_{expand_ratio}_seed_{seed}_max_tokens_{max_new_tokens}_samples_{num_samples}_skip_{skip_num}_random_{random_int}_generated_captions.json",
    )
...
config_captions_path = os.path.join(
    base_dir,
    f"{model_name}_{decoding_strategy}_{detector_type}_box_{box_threshold}_beams_{num_beams}_k_{k_candidate_num}_{dataset_name}_expand_ratio_{expand_ratio}_seed_{seed}_max_tokens_{max_new_tokens}_samples_{num_samples}_skip_{skip_num}_random_{random_int}_config.json",
)
with open(config_captions_path, "a") as f:
    json.dump(findings_kwargs, f)
    f.write("\n")
